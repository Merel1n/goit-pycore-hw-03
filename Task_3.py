...
# коли номери телефонів представлені у коректному форматі. 
...
# Функція приймає один аргумент - рядок з телефонним номером у будь-якому форматі та перетворює його на 
# стандартний формат, залишаючи тільки цифри та символ '+'. 
...
# враховуючи наявність або відсутність міжнародного коду.
...
def normalize_phone(phone_number : str) -> str:
    ...
    # Створюємо патерн які символи треба видалити
    pattern = r"[^\d+]"
    # Чистимо вхідний рядок з номерами
    cleaned = re.sub(pattern, "", clean_phone_number)
    # Перевіряємо і виправляємо усі номери в правильний формат
    # Перевіряємо чи починається з "+"
    if cleaned.startswith('+'):
        # Перевіряємо чи починається з "+38" якщо так то залишаємо так як є
        if cleaned.startswith('+38'):
            return cleaned
        # Перевіряємо чи починається з "+380" якщо так залишаємо так як є
        elif cleaned.startswith('+380'):
            return cleaned
        # Перевіряємо чи номер є міжнародним
        else:
            return cleaned
        # Перевіряємо чи номер починається з "380" якщо так додаємо "+" на початку
    elif cleaned.startswith('380'):
        return f'+{cleaned}'
    # Якщо номер починається з 0 то додаємо "+38"
    else:
        return f'+38{cleaned}'
